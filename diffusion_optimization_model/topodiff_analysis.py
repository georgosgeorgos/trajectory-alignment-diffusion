...
def topodiff_analysis(num_samples, num_folder, summaries, gen_dir, flag="dom"):

    lst = order_list(num_samples, num_folder)
    summaries = pre_process_summaries(summaries, lst)
    
    if flag == "plus_simp":
        gen = np.load(gen_dir + f"samples_{num_samples}x64x64x1_new.npy")
    else:
        gen = np.load(gen_dir + f"samples_{num_samples}x64x64x1.npz")["arr_0"]
        
    # dataset with intermediate with 0,1 inverted
    gen  = (1 - gen / 255.) * 255.
    #
    gen[gen < 127] = 0.
    gen[gen >= 128] = 255.
