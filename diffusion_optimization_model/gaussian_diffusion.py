...
#             temp = out["mean"]
    ...
    def p_sample_loop_progressive_store_intermediate(
        self,
        model,
        shape,
        cons,
    ...
    def training_losses(self, model, regressor, x_start, cons, x_inter, x_perf, t, model_kwargs=None, noise=None, consistency_terms=[]):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # compute consistency in configuration space
            pred_xstart = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=model_output)
            pred_xstart = pred_xstart.clamp(-1, 1)

            # multifidelity
            if pred_xstart.shape[-1] !=x_inter.shape[-1]:
                pred_xstart = nnf.interpolate(pred_xstart, size=(256, 256), mode='bicubic', align_corners=False)
                terms["consistency_multi"] = mean_flat((x_inter - pred_xstart) ** 2) / 100
            else:
                terms["consistency"] = mean_flat((x_inter - pred_xstart) ** 2) / 100
            

            if "consistency_multi" not in consistency_terms:
                # compute consistency in noisy space
                opt_xt = self.q_sample(x_inter, t, noise=noise)
                opt_xt = opt_xt.clamp(-1, 1)

                # compute consistency in performance space
                pred_xt, _, _ = self.q_posterior_mean_variance(
                    x_start=pred_xstart, x_t=x_t, t=t
                )
                pred_xt = pred_xt.clamp(-1, 1)
                
                terms["consistency_noisy"] = mean_flat((opt_xt - pred_xt) ** 2) / 100
                
                if regressor:
                    pred_perf = regressor(th.cat((pred_xt, cons), dim = 1), t)
                    opt_perf = regressor(th.cat((x_inter, cons), dim = 1), t)
                    #print(pred_perf, opt_perf, x_perf)
                    terms["consistency_performance0"] = mean_flat(th.abs(opt_perf - x_perf)) / 100
                    terms["consistency_performance1"] = mean_flat(th.abs(pred_perf - opt_perf))
            ...
            if "vb" in terms:
                ...
                for ct in consistency_terms:
                    terms["loss"] += terms[ct]
                # + terms["consistency"] + terms["consistency_noisy"] + terms["consistency_performance0"] + terms["consistency_performance1"]
            else:
