...
from diffusion_optimization_model.cons_input_datasets import load_data
...
def main():
    ...
    while len(all_images) * args.batch_size < args.num_samples:
        ...
        all_bc.extend(input_raw_BCs.cpu().numpy())
        
    arr = np.concatenate(all_images, axis=0)
    arr_bc = np.concatenate(all_bc, axis=0)
    ...
    if dist.get_rank() == 0:
        ...
        out_path_bc = os.path.join(logger.get_dir(), f"samples_bc_{shape_str}.npz")
        np.savez(out_path_bc, arr_bc)
